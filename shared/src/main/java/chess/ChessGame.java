...
public class ChessGame {
    ...
    private boolean testCheck(ChessBoard board, TeamColor teamColor) {
        if (teamColor == TeamColor.WHITE) {
            blackThreatens();
            for (ChessMove i : blackValid) {
                if (i.getEndPosition().equals(getKing(board, teamColor))) {
                    return true;
                }
            }
        }
